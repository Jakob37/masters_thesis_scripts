...
# Helena Liljedahl's script based on https://github.com/meoyo/trainAIMS
# Updated: 191009
...
.comp.sel.pairs <- function(dataset,sel.pairs,func=.smaller){
  ...
  if(length(sel.pairs)==1){
    to.ret <- t(as.matrix(apply(to.ret,2,as.numeric)))
...
## Remove the duplicated Entrez by keeping the most highly expressed
